...
    }
   
    // ⚠️ DEBES IMPLEMENTAR LA LÓGICA DE CACHÉ Y BASE DE DATOS AQUÍ.
    // Esto mejorará el rendimiento y evitará la extracción repetida.
    // Ejemplo (comentado):
...
    // 1. Intentar con los extractores específicos que ya tienes (método rápido)
...
    // 2. Si falló el primer intento, usar yt-dlp (segundo recurso)
    if (!videoUrl) {
        console.log('Fallo el extractor específico. Intentando con yt-dlp...');
        try {
...
            });
        } catch (e) {
            console.error('Fallo el método yt-dlp:', e.message);
        }
    }
...
            });
        } catch (e) {
            console.error('Fallo el método Puppeteer:', e.message);
        } finally {
            await browser.close();
        }
    }
    
    if (videoUrl) {
        // ✅ Guardar en caché (DEBES IMPLEMENTAR LA LÓGICA REAL DE LA DB)
        // const expirationTime = new Date(Date.now() + 24 * 60 * 60 * 1000);
        // await db.saveLink(url, videoUrl, expirationTime);
        res.json({ videoUrl });
    } else {
