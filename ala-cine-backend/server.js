...
`;
            ...
            // +++ CORRECCIÓN CRÍTICA: Lógica para "Agregar Siguiente Episodio" +++
                ...
                // 1. Verificación de integridad de datos clave
                if (!state || !state.lastSavedEpisodeData || state.lastSavedEpisodeData.tmdbId !== tmdbId) {
                     bot.sendMessage(chatId, 'Error: Datos perdidos. Vuelve a empezar la gestión de esta serie.', { parse_mode: 'Markdown' });
                     adminState[chatId] = { step: 'menu' };
                     return;
                }

                // 2. Cálculo del siguiente episodio
                const nextEpisode = state.lastSavedEpisodeData.episodeNumber + 1;
                
                // 3. Establecer el estado para el siguiente paso
                //    Mantenemos el estado (con selectedSeries) y actualizamos la secuencia.
                adminState[chatId] = {
                    ...state, 
                    step: 'awaiting_pro_link_series',
                ...
                // 4. Solicitamos el link PRO para el nuevo episodio
                bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: chatId, message_id: msg.message_id }).catch(() => {});
                bot.sendMessage(chatId, `Siguiente: Envía link PRO para S${season}E${nextEpisode} (o "no").`, { parse_mode: 'Markdown' });
            }
            ...
            // --- FIN NUEVO CALLBACK: GUARDAR + PUSH + CANAL (PELÍCULAS) ---
                ...
                // ESTE BLOQUE HA SIDO MOVIDO Y CORREGIDO ARRIBA
                // El error de "datos perdidos" ocurría porque este handler no existía.
            }
            else if (data.startsWith('publish_push_this_episode_')) {
                const [_, __, ___, tmdbId, season, episode] = data.split('_');
                const state = adminState[chatId];
                // ⚠️ Se asume que lastSavedEpisodeData existe si el flujo es correcto.
                ...
                if (!episodeData || episodeData.tmdbId !== tmdbId || episodeData.seasonNumber.toString() !== season || episodeData.episodeNumber.toString() !== episode) {
                    bot.sendMessage(chatId, 'Error: Datos perdidos. Vuelve a empezar la gestión.', { parse_mode: 'Markdown' }); 
                    adminState[chatId] = { step: 'menu' }; 
                    return; 
                }
            ...
            // +++ NUEVO CALLBACK: GUARDAR + PUSH + CANAL (SERIES) +++
                ...
                const episodeData = state?.lastSavedEpisodeData;
                if (!episodeData || episodeData.tmdbId !== tmdbId || episodeData.seasonNumber.toString() !== season || episodeData.episodeNumber.toString() !== episode) {
                    bot.sendMessage(chatId, 'Error: Datos perdidos. Vuelve a empezar desde el episodio anterior.', { parse_mode: 'Markdown' }); 
                    adminState[chatId] = { step: 'menu' }; 
                    return;
                }
                bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: chatId, message_id: msg.message_id });
