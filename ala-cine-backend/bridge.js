...
module.exports = function(app) {
    ...
    const PLAY_STORE_URL = "https://play.google.com/store/apps/details?id=com.salacine.app"; // Ajusta esto si cambia
    ...
    async function getTmdbData(id, type) {
        ...
        try {
            // Solicitamos detalles, créditos, videos, imágenes y proveedores en una sola llamada (append_to_response)
    ...
    app.get('/view/:type/:id', async (req, res) => {
        ...
        // Proveedores (Flatrate = Streaming por suscripción)
        const providers = data['watch/providers']?.results?.MX?.flatrate || []; // Usamos MX por defecto, puedes ajustar
        ...
        // Esquema Deep Link
        const appScheme = `salacine://details?id=${id}&type=${type}`; // Asegúrate que tu App soporte &type=

        // --- RENDERIZADO HTML (Server Side Rendering puro) ---
        const html = `
        <!DOCTYPE html>
        <html lang="es">
        <head>
            ...
            <style>
                ...
                .iframe-cover { position: absolute; top:0; left:0; width:100%; height:100%; z-index: 5; background: transparent; } /* Evita click en youtube */
            ...
            <script>
                function openApp() {
                    ...
                    setTimeout(function () {
                        ...
                        // Si no se abrió la app, redirigir a Play Store
                ...
                // Intentar abrir automáticamente al cargar (Opcional, a veces molesto, mejor dejar al click)
